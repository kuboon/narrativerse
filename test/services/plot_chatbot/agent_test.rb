...
describe PlotChatbot::Agent do
  ...
  it "applies plot-aware instructions and tools" do
    PlotChatbot::Agent.new(chat:, plot_id: plot.id, scene_id: 123, persist_instructions: false)
    ...
    _(tool_classes).must_include PlotChatbot::Tools::ListPlotElementsTool
    _(tool_classes).must_include PlotChatbot::Tools::PresentChoicesTool
  end

  it "uses generic instructions and no tools without plot context" do
    PlotChatbot::Agent.new(chat:, persist_instructions: false)
  ...
  it "executes configured tools in rails mode" do
    PlotChatbot::Agent.new(chat:, plot_id: plot.id, persist_instructions: false)

    llm_tools = chat.to_llm.tools.values

    list_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::ListPlotElementsTool) }
    list_payload = JSON.parse(list_tool.execute)
    _(list_payload["status"]).must_equal("ok")

    add_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::AddPlotElementTool) }
    ...
    update_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::UpdatePlotElementTool) }
    ...
    search_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::SearchElementsTool) }
    ...
    list_scenes_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::ListScenesTool) }
    ...
    add_scene_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::AddSceneTool) }
    add_scene_payload = JSON.parse(add_scene_tool.execute(text: "次のシーン"))
    _(add_scene_payload["status"]).must_equal("ok")

    update_scene_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::UpdateSceneTool) }
    ...
    choices_tool = llm_tools.find { |tool| tool.is_a?(PlotChatbot::Tools::PresentChoicesTool) }
