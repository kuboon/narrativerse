    ...
    module Mcp
      class Base < MCP::Tool
        ...
        class << self
          ...
          def schema
            json_schema = pure_tool_class.schema
            MCP::Tool::InputSchema.new(json_schema)
          end
          ...
          def call_wrapped(arguments:, server_context:)
            ...
            tool = pure_tool_class.new(user: context[:user], plot: context[:plot])
            payload = normalize_payload(tool.execute(**arguments))
